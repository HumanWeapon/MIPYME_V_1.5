...
export class PermisosService {
  ...
  //consulta los permisos de los roles para poder acceder a las rutas.
  getPermnisosObjetos(id_rol: string, id_objeto: string): Observable<any[]> {
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
    return this.http.get<any[]>(`${this.myAppUrl}${this.myApiUrl}/getPermnisosObjetos/${id_rol}/${id_objeto}`, { headers: headers });
  }
}
